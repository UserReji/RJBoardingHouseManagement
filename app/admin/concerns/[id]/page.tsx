...
import { ChevronLeft, AlertCircle, User, ImageIcon } from "lucide-react";
...
export default async function ConcernDetailPage({
  ...
  const tenant = Array.isArray((concern as any).users) ? (concern as any).users[0] : (concern as any).users;
    ...
    <div className="page md:p-6">
      <div className="page-content md:max-w-full md:p-0">
        <div className="flex items-center gap-3 mb-6">
          <Link href="/admin/concerns" className="btn btn-ghost btn-sm">
          ...
          <div className="flex items-start gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                <Link href={`/admin/tenants/${tenant?.id}`} className="hover:text-blue-600">{tenant?.full_name}</Link>
                {" · "}{fmt(concern.created_at as any)}
              </p>
            </div>
        ...
        <div className="mb-5">
          ...
          {replies.length === 0 ? (
            <div className="card p-6 text-center text-sm text-slate-500">No replies yet.</div>
          ) : (
                ...
                <li key={r.id} className={`card p-4 ${
                  ...
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className={`badge ${r.sender_role === "admin" ? "badge-info" : "badge-slate"}`}>
                      {r.sender_role === "admin" ? "Admin" : "Tenant"}
                    </span>
        ...
        {/* Actions */}
        <ConcernActions concernId={concern.id} currentStatus={concern.status as any} />
      </div>
    </div>
  );
}
